<nav class="navbar navbar-expand-lg navbar-light p-4" style="background-color: #FFE57C;">
            <div class="container px-4 px-lg-5 ">
                <button class="navbar-toggler btn btn-sm" type="button" data-toggle="collapse" data-target="#navbarSupportedContent" aria-controls="navbarSupportedContent" aria-expanded="false" aria-label="Toggle navigation"><span class="navbar-toggler-icon"></span></button>
                <a class="navbar-brand text-black" href="./">
                <!-- <img src="<?php echo validate_image($_settings->info('logo.jpg')) ?>" width="30" height="30" class="d-inline-block align-top" alt="" loading="lazy"> -->
                ...
                <form class="form-inline" id="search-form">
                  <div class="d-flex">
                    ...
                    <div class="input-group-append">
                      <button style="background-color: #Ffffff;border-color: #000" class="btn btn-sm m-0" type="submit" id="button-addon2"><i class="fa fa-search"></i></button>
                    </div>
                  </div>
                </form>
                <div class="collapse navbar-collapse" id="navbarSupportedContent">
                    <ul class="navbar-nav me-auto mb-2 mb-lg-0 ms-lg-4">
                        <li class="nav-item"><a class="nav-link text-black" aria-current="page" href="./">Home</a></li>
                        <?php 
                       ...
                       <li class="nav-item dropdown">
                          <a class="nav-link text-black dropdown-toggle" id="navbarDropdown<?php echo $crow['id'] ?>" href="#" role="button" data-toggle="dropdown" aria-expanded="false"><?php echo $crow['category'] ?></a></a>
                            <ul class="dropdown-menu  p-0" aria-labelledby="navbarDropdown<?php echo $crow['id'] ?>">
                              <?php while($srow = $sub_qry->fetch_assoc()): ?>
                                <li><a style="color: #000" class="dropdown-item border-bottom" href="./?p=products&c=<?php echo md5($crow['id']) ?>&s=<?php echo md5($srow['id']) ?>&sub_category_id=<?php echo $srow['id'] ?>"><?php echo $srow['sub_category'] ?></a></li>
                            <?php endwhile; ?>
                            </ul>
                        </li>
                        ...
                        <?php endif; ?>
                        <?php endwhile; ?>
                        <li class="nav-item"><a class="nav-link text-black" href="./?p=about">About</a></li>
                    </ul>
                    <div class="d-flex align-items-center">
                      <?php if(!isset($_SESSION['userdata']['id'])): ?>
                        <button style="border-color: #fff;background-color: #FFE57C;" class="btn text-dark ml-2" id="login-btn" type="button">Login</button>
                        <?php else: ?>
                        <a class="text-dark mr-2 nav-link" href="./?p=cart">
                            <i class="bi-cart-fill me-1"></i>
                            Cart
                            <span class="badge bg-black text-black ms-1 rounded-pill" id="cart-count">
                              <?php 
                            ...
                            <a style="color: #000;" href="logout.php" class="text-  nav-link"><i class="fa fa-sign-out-alt"></i></a>
                        <?php endif; ?>
                    </div>
                </div>
            </div>
