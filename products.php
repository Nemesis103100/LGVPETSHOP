        ...
        <div class="row gx-4 gx-lg-5 row-cols-2 row-cols-md-3 row-cols-xl-4 justify-content-center">
            ...
            <?php 
                ...
                if (isset($_GET['category_id']))
                    $products = $conn->query("SELECT * FROM `products` where status = 1 and category_id = '".$_GET['category_id']."' order by id limit 12 ");
                elseif(isset($_GET['sub_category_id']))
                    $products = $conn->query("SELECT * FROM `products` where status = 1 and sub_category_id = '".$_GET['sub_category_id']."' order by id limit 12 ");
                elseif(!isset($_GET['category_id']))
                    $products = $conn->query("SELECT * FROM `products` where status = 1 order by id limit 12 ");
            ...
            <div class="col mb-5">
                <div class="card h-100 product-item">
                    ...
                    <div class="card-footer p-4 pt-0 border-top-0 bg-transparent">
                        <div class="text-center">
                            <a style="background-color: #FFE57C;" class="btn btn-flat "   href=".?p=view_product&id=<?php echo md5($row['id']) ?>">View</a>
                        </div>
